...
export async function startPayment(
  sdk,
  { amount, externalUserId, currency = 'usd', metadata = {} },
) {
  try {
...
async function initializeSDK(apiKey) {
  try {
    const response = await fetch(`${process.env.API_URL}/v1/payment/sdk/init`, {
      method: 'POST',
      headers: {
        'x-api-key': apiKey,
      },
...
export async function createPaymentIntent(sdk, { amount, currency, metadata }) {
  try {
    if (!sdk.encryptionKey) {
      ...
      if (!sdk.encryptionKey) {
        throw new Error(
          'Encryption key not available. SDK not properly initialized.',
        );
      }
    }
    ...
    const response = await fetch(
      `${process.env.API_URL}/v1/payment/sdk/create-order`,
      {
        method: 'POST',
        headers: {
